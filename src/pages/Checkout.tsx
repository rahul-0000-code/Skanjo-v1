...
import { z } from "zod";
import { Check, CreditCard, Key, Copy, CheckCircle, Shield, Clock, Star, Users, Zap, Award, AlertCircle } from "lucide-react";
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    if (isPollingPayment && paymentOrder?.order_id && user?.api_key) {
      pollInterval = setInterval(async () => {
        try {
          const status = await getPaymentStatus(user.api_key, paymentOrder.order_id);
  ...
  }, [isPollingPayment, paymentOrder?.order_id, user?.api_key, plan.name, toast]);
  ...
  const handleProfileSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      if (plan.price === 'Free') {
        try {
          const featureKeyRes = await createFeatureKey(user.api_key, undefined, 'free');
          setApiKey(featureKeyRes.feature_key);
      ...
      const options: any = {
        ...
        handler: function (response: any) {
          ...
          toast({
            ...
            description: "Payment was successful. Verifying...",
          });
        },
        prefill: {
          name: user.name,
